...
- [ ] 更新首頁 `docs/index.md` 的最新文章區塊（見下方說明）

#### 步驟 6：更新首頁最新文章區塊

每次新增文章後，需要更新首頁的「📝 最新文章」區塊，顯示**跨所有分類最新的 5 篇文章**。

**更新規則：**

1. **找出最新的 5 篇文章**：從 AI、Robot、Project 三個分類中，依照日期排序，選出最新的 5 篇
2. **按日期由新到舊排列**：最新的文章放在最上方
3. **包含完整資訊**：
   - 文章標題（連結）
   - 分類標籤（AI / 機器人 / 專案）
   - 發布日期
...
- 日期格式統一使用 `YYYY-MM-DD`（例如：2025-12-05）
- 分類標籤使用：`AI`、`機器人`、`專案`
- 摘要控制在 1-2 句話，避免過長
- 使用 `---` 分隔線區隔不同文章
- 確保連結路徑正確（使用絕對路徑，如 `/ai/article-name`）
